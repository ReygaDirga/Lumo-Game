...
function update(){
  ...
  let baseSpeed = 10 + Math.floor(elapsed/5) * 4;
  ...
  gameState.speed = Math.min(gameState.speed, 50);
  ...
  for (let p of gameState.powerups){
    if (photon.lane===p.lane && Math.abs(photon.x-p.x)<30){
      ...
      if (p.type==="slow") {
        gameState.slowEffect = 3;
        gameState.slowTimer = Date.now() + 6000; 
      }
      if (p.type==="fragment") {
      gameState.fragments++;
